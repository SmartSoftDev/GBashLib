...
def main():
    ...
    if args.local:
        ...
        if env_name:
            file_name = "v.{env_name}.yaml".format(env_name=env_name)
    ...
    if args.cmd == "set":
        ...
        gpg_usr_s_id = [
            {"id": user_key["fingerprint"], "uids": user_key["uids"]}
            for user_key in gpg.list_keys()
        ...
        if name in config:
            raise ValueError(
                "Field value is already Exists! if you want to reEncrypt run v add-user-key or v del-user-key"
            )
        ...
        if len(gpg_usr_s_id) != len(args.recipients):
            parser.error(
                message="Didn't find user keys (%r out of %r) for passed %r!"
                % (len(gpg_usr_s_id), len(args.recipients), args.recipients)
            )
        ...
        if args.key_name is None:
            ...
            if value and isinstance(value, dict) and value.get("enc_type") == "gpg":
                ...
                if len(value_keys) != len(found_keys):
                    ...
                    raise Exception(
                        "Could not find all (%r of %r [%r]) keys to re/encrypt name=%r"
                        % (len(found_keys), len(value_keys), value_keys, name)
                    )
