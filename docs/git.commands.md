...
* gd='git diff'
* gl="git log --pretty=format:'%C(dim)%h%Creset  %C(green)%s%Creset %C(yellow)<%an>%Creset %C(dim)%cr(%cs)%Creset %C(blue)%D%Creset' --name-only"  - short view of git history which shows also the affected files.
...
* `g type` = cat-file -t
* `g dump` = cat-file -p
* `g l`= log --pretty=format:'%C(dim)%h%Creset  %C(green)%s%Creset %C(yellow)<%an>%Creset %C(dim)%cr(%cs)%Creset %C(blue)%D%Creset' --name-only
